...
public class WaypointSettingsScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (e.isHeader) {
                g.centeredText(this.font, Component.translatable(e.labelKey), this.width / 2, rowY + 6, 0xFFFFD34F);
            } else {
                g.text(this.font, Component.translatable(e.labelKey), textX, textY, 0xFFFFFFFF, false);
            }
        }

        for (Button b : rowButtons) {
            if (b.visible) b.extractRenderState(g, mouseX, mouseY, partialTick);
        }
        ...
        cancelBtn.extractRenderState(g, mouseX, mouseY, partialTick);
    }
