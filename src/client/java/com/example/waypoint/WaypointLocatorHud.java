...
int hx1 = (c.drawX - halfBase) + HOVER_SHRINK_X;
    ...
    private static void drawCompassScale(GuiGraphicsExtractor g, Minecraft mc, int barX, int barY, int centerX) {
        float playerYaw = mc.player.getYRot();
        ...
        for (int deg = -180; deg <= 180; deg += COMPASS_MINOR_EVERY) {
            ...
            if (showDirs && (deg % COMPASS_LABEL_EVERY == 0)) {
                ...
                g.text(mc.font, label, sx, sy, 0xFFFFFFFF, true);
    ...
    private static void drawCenterLabel(GuiGraphicsExtractor g, Minecraft mc, Cluster c, int centerX, int barY, int sw) {
        ...
        if (c.entries.size() == 1) {
            ...
            g.text(mc.font, s, sx, sy, col, false);
        ...
        for (int i = 0; i < totalLines; i++) {
            g.text(mc.font, lines[i], sx, y, colors[i], false);
            y += lineStep;
        }
...
private static void fillTriangle(GuiGraphicsExtractor g, int cx, int y1, int y2, int halfBase, int argb, boolean up) {
